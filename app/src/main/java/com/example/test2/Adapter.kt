...
import com.example.test2.databinding.ItemAppBinding
...
//        holder.appBinding.appIcon.setImageDrawable(appList?.get(position)?.icon)
        ...
        holder.appBinding.root.setOnClickListener {
            context.startActivity(
                context.packageManager.getLaunchIntentForPackage(appList?.get(position)?.packageName?:"com.example.test2")
            )
        }
    }
